package epimed_web.controller.ajax;
...
import epimed_web.service.mongodb.jobs.JobService;
...
public class AjaxController extends ApplicationLogger {
	...
	public AjaxResponse upload (
		...
		try  {
			...
			if (currentIndex==null || currentIndex.equals(0)) {
				logger.debug("*** Init ***");
				currentIndex=0;
				jobid = jobService.createJob(jobtype, listElements, request);
			...
			for (String processedSymbol: symbols) {
				...
				boolean isFound = jobElementService.updateJobElements(jobid, processedSymbol, taxid, 30, ajaxForm);
				...
				if (!isFound) {
					...
					jobElementService.createJobElments(jobid, processedSymbol, taxid, 30, ajaxForm);
				}
				...
				String message = "NOT FOUND " + processedSymbol;
