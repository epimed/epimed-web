...
public interface ProbesetRepository extends GraphRepository<Probeset> {
	...
	@Query("MATCH (p:Probeset)-[*1..2]->(g:Gene) WHERE g.uid={idGene} AND {idPlatform} IN p.id_platforms RETURN p.uid")
	public Set<String> findByGene(@Param("idGene") Integer idGene, @Param("idPlatform") String idPlatform);
	
	@Query("MATCH (p:Probeset)-[*1..3]->(n:Nucleotide) WHERE n.uid={symbol} AND {idPlatform} IN p.id_platforms RETURN p.uid")
	public Set<String> findByNucleotideSymbol(@Param("symbol") String symbol, @Param("idPlatform") String idPlatform);
